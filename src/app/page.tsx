...
// دالة للحصول على العنوان المناسب حسب النوع
...
export default function WatchListPage() {
  ...
  const allGenres = useMemo(() => { const g = new Set<string>(); watchList.forEach(i => i.genres?.forEach((x: string) => g.add(x))); return Array.from(g).sort() }, [watchList])

  const filteredItems = useMemo(() => {
    let items = activeTab === 'all' ? watchList : watchList.filter(i => i.type === activeTab)
  ...
  const stats = useMemo(() => { const items = activeTab === 'all' ? watchList : watchList.filter(i => i.type === activeTab); return { total: items.length, watched: items.filter(i => i.watched).length, favorite: items.filter(i => i.favorite).length, avgRating: items.reduce((a, i) => a + (parseFloat(i.rating) || 0), 0) / (items.length || 1) } }, [watchList, activeTab])
  const tabStats = useMemo(() => ({ all: { total: watchList.length, watched: watchList.filter(i => i.watched).length }, anime: { total: watchList.filter(i => i.type === 'anime').length, watched: watchList.filter(i => i.type === 'anime' && i.watched).length }, series: { total: watchList.filter(i => i.type === 'series').length, watched: watchList.filter(i => i.type === 'series' && i.watched).length }, movie: { total: watchList.filter(i => i.type === 'movie').length, watched: watchList.filter(i => i.type === 'movie' && i.watched).length }, book: { total: watchList.filter(i => i.type === 'book').length, watched: watchList.filter(i => i.type === 'book' && i.watched).length } }), [watchList])
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const data = await res.json()
      if (data) { setWatchList(p => p.map(i => i.id === editingItem.id ? data : i)); setSyncStatus('synced') }
      setShowEditDialog(false); setEditingItem(null); resetForm()
    } catch { setSyncStatus('error') }
  }

  const removeFromList = async (id: string) => { try { setSyncStatus('syncing'); await fetch(`/api/watchlist/${id}`, { method: 'DELETE' }); setWatchList(p => p.filter(i => i.id !== id)); setSyncStatus('synced'); if (selectedItem?.id === id) { setShowDetails(false); setSelectedItem(null) } } catch { setSyncStatus('error') } }
  const toggleWatched = async (id: string) => { const item = watchList.find(i => i.id === id); if (!item) return; try { await fetch(`/api/watchlist/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ watched: !item.watched }) }); setWatchList(p => p.map(i => i.id === id ? { ...i, watched: !i.watched } : i)) } catch {} }
  const toggleFavorite = async (id: string) => { const item = watchList.find(i => i.id === id); if (!item) return; try { await fetch(`/api/watchlist/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ favorite: !item.favorite }) }); setWatchList(p => p.map(i => i.id === id ? { ...i, favorite: !i.favorite } : i)) } catch {} }
  ...
  const exportData = () => { const d = JSON.stringify(watchList, null, 2); const b = new Blob([d], { type: 'application/json' }); const u = URL.createObjectURL(b); const a = document.createElement('a'); a.href = u; a.download = `watchlist_${new Date().toISOString().split('T')[0]}.json`; a.click(); URL.revokeObjectURL(u) }
  const importData = async (e: React.ChangeEvent<HTMLInputElement>) => { const f = e.target.files?.[0]; if (f) { const r = new FileReader(); r.onload = async (ev) => { try { const imp = JSON.parse(ev.target?.result as string); if (Array.isArray(imp)) { for (const item of imp) { await fetch('/api/watchlist', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(item) }) } fetchWatchList() } } catch {} }; r.readAsText(f) } }
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-8" dir="rtl">
        <header className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-5"><div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#d4af37] to-[#b8960f] flex items-center justify-center shadow-lg"><Bookmark className="w-7 h-7 text-[#0a0a0a]" /></div><div><h1 className="text-2xl font-bold">أرشيفي</h1><div className="flex items-center gap-2"><p className="text-neutral-500 text-sm">قائمة المشاهدات والقراءة</p><span className={`text-xs flex items-center gap-1 ${syncStatus === 'synced' ? 'text-green-500' : syncStatus === 'syncing' ? 'text-yellow-500' : 'text-red-500'}`}>{syncStatus === 'synced' && <Cloud className="w-3 h-3" />}{syncStatus === 'syncing' && <Loader2 className="w-3 h-3 animate-spin" />}{syncStatus === 'error' && <CloudOff className="w-3 h-3" />}{syncStatus === 'synced' ? 'متزامن' : syncStatus === 'syncing' ? 'مزامنة...' : 'خطأ'}</span></div></div></div>

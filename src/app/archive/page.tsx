      ...
      <div className="min-h-screen bg-[#0a0a0a] text-white flex items-center justify-center">
        <div className="w-full max-w-sm px-6">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold mb-2">أريد مشاهدته</h1>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-8" dir="rtl">
        <header className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-5">
            ...
            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#d4af37] to-[#b8960f] flex items-center justify-center shadow-lg"><Bookmark className="w-7 h-7 text-[#0a0a0a]" /></div><div><h1 className="text-2xl font-bold">أريد مشاهدته</h1><div className="flex items-center gap-2"><p className="text-neutral-500 text-sm">قائمة المشاهدات</p><span className={`text-xs flex items-center gap-1 ${syncStatus === 'synced' ? 'text-green-500' : syncStatus === 'syncing' ? 'text-yellow-500' : 'text-red-500'}`}>{syncStatus === 'synced' && <Cloud className="w-3 h-3" />}{syncStatus === 'syncing' && <Loader2 className="w-3 h-3 animate-spin" />}{syncStatus === 'error' && <CloudOff className="w-3 h-3" />}{syncStatus === 'synced' ? 'متزامن' : syncStatus === 'syncing' ? 'مزامنة...' : 'خطأ'}</span></div></div></div>

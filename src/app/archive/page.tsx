...
const FIRST_LETTER_OPTIONS = [
  'أ', 'ا', 'ب', 'ت', 'ث', 'ج', 'ح', 'خ', 'د', 'ذ', 'ر', 'ز', 'س', 'ش', 'ص', 'ض', 'ط', 'ظ', 'ع', 'غ', 'ف', 'ق', 'ك', 'ل', 'م', 'ن', 'ه', 'و', 'ي',
    ...
    return (
      ...
      >
        ...
        <div className="flex items-center gap-1 shrink-0" onClick={e => e.stopPropagation()}>
          <button
            onClick={onQuickRate}
            className="w-10 h-10 rounded-full bg-[#d4af37]/15 text-[#d4af37] flex items-center justify-center active:scale-90 transition-transform"
            title="تقييم"
          >
...
const fetchWatchlist = useCallback(async (page: number, reset = false) => {
  ...
  try {
    ...
    if (wlFilterRatingMin || wlFilterRatingMax) params.set('ratingSource', 'rating')
...
const fetchRatings = useCallback(async (page: number, reset = false) => {
  ...
  try {
    ...
    if (rtFilterRatingMin || rtFilterRatingMax) params.set('ratingSource', 'userRating')
    params.set('page', String(page))
    params.set('limit', '200')
    const res = await fetch(`/api/watchlist?${params}`)
    const data = await res.json()
  ...
  const searchMetadata = useCallback(async (query?: string) => {
    ...
    try {
      ...
      setMetaLoading(false)
      // Restore focus to the search input after results load
      setTimeout(() => metaInputRef.current?.focus(), 50)
    }
  }, [metaQuery, formData.type, MIN_SEARCH_CHARS])
  ...
  const LetterFilter = ({
    ...
    <div>
      <h4 className="text-xs font-bold text-[#d4af37] mb-2">الحرف الأول</h4>
      <div className="flex gap-1.5 flex-wrap max-h-32 overflow-y-auto">
        ...
        {FIRST_LETTER_OPTIONS.map(letter => (
          <button
            key={letter}
            onClick={() => onChange(letter === value ? '' : letter)}
            className={`px-2.5 py-1.5 rounded-md text-xs font-medium transition-colors ${
              letter === value
                ? 'bg-[#d4af37]/15 text-[#d4af37] border border-[#d4af37]/30'
                : 'bg-[#0a0a0a] text-[#888] border border-[#2a2a2a] hover:text-[#ccc]'
  ...
  const RatingRangeFilter = useCallback(({
    ...
    <div>
      <h4 className="text-xs font-bold text-[#d4af37] mb-2">التقييم</h4>
  ...
  const wlSortFilterContent = (
    <div className="space-y-4 p-4" dir="rtl">
      ...
      <LetterFilter value={wlFilterFirstLetter} onChange={setWlFilterFirstLetter} />
      ...
      <RatingRangeFilter
        min={wlFilterRatingMin}
        max={wlFilterRatingMax}
        onMinChange={setWlFilterRatingMin}
        onMaxChange={setWlFilterRatingMax}
        maxValue={10}
      />
  ...
  const rtSortFilterContent = (
    <div className="space-y-4 p-4" dir="rtl">
      ...
      <LetterFilter value={rtFilterFirstLetter} onChange={setRtFilterFirstLetter} />
      ...
      <RatingRangeFilter
        min={rtFilterRatingMin}
        max={rtFilterRatingMax}
        onMinChange={setRtFilterRatingMin}
        onMaxChange={setRtFilterRatingMax}
        maxValue={100}
      />
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          ...
          <div className="absolute left-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
            {metaLoading && <Loader2 className="w-4 h-4 animate-spin text-[#d4af37]" />}
        ...
        {metaLoading && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 p-1">
            {[1, 2, 3].map(i => (
              <div key={i} className="flex items-center gap-3 p-2.5 rounded-xl bg-[#1a1a1a] border border-[#2a2a2a]">
                <div className="w-11 h-16 rounded-lg bg-[#2a2a2a] skeleton-shimmer shrink-0" />
        ...
        {!metaLoading && !metaError && metaQuery.trim().length >= MIN_SEARCH_CHARS && metaResults.length === 0 && (
          <div className="flex items-center gap-2 p-3 rounded-xl bg-[#1a1a1a] border border-[#2a2a2a] text-[#888] text-xs">
      ...
      <main className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-4 pb-6">
        ...
        {mainTab === 'watchlist' && (
          <div className="space-y-4">
            ...
            {/* Year Filter Bar — prominent horizontal scrollable chips (desktop only) */}
            {dbYears.length > 0 && (
              ...
              <div className="flex items-center gap-1.5">
                ...
                {deviceType === 'mobile' ? (
                  ...
                  >
                    ...
                    <span>فلاتر{wlActiveFilterCount > 0 ? ` (${wlActiveFilterCount})` : ''}</span>
                  </Button>
                ) : (
                  <Popover open={wlShowSortFilter} onOpenChange={setWlShowSortFilter}>
                    <PopoverTrigger asChild>
                      ...
                      >
                        ...
                        {wlActiveFilterCount > 0 && (
                          <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-[#d4af37] text-black text-[10px] font-bold shrink-0">{wlActiveFilterCount}</span>
                        )}
                      </Button>
                    </PopoverTrigger>
            ...
            {debouncedSearch ? rtFilteredItems.length : rtTotal} عمل مقيّم
              ...
              <div className="flex items-center gap-1.5">
                ...
                {deviceType === 'mobile' ? (
                  ...
                  >
                    <SlidersHorizontal className="w-3.5 h-3.5" />
                    <span>فلاتر{rtActiveFilterCount > 0 ? ` (${rtActiveFilterCount})` : ''}</span>
                  </Button>
                ) : (
                  <Popover open={rtShowSortFilter} onOpenChange={setRtShowSortFilter}>
                    <PopoverTrigger asChild>
                      ...
                      >
                        ...
                        {rtActiveFilterCount > 0 && (
                          <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-[#d4af37] text-black text-[10px] font-bold shrink-0">{rtActiveFilterCount}</span>
                        )}
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-80 bg-[#1a1a1a] border-[#2a2a2a] p-0" align="start" dir="rtl">

    ...
    return (
      ...
      >
        ...
        <div className="flex items-center gap-1 shrink-0" onClick={e => e.stopPropagation()}>
          <button
            onClick={onQuickRate}
            className="w-10 h-10 rounded-full bg-teal-500/15 text-teal-400 flex items-center justify-center active:scale-90 transition-transform"
            title="تقييم"
          >
  ...
  const renderForm = (_isEdit: boolean) => (
    <div className="space-y-4 p-1" dir="rtl">
      ...
      <div className="space-y-2">
        <label className="text-sm font-bold text-teal-400 flex items-center gap-1.5">
          البحث عن لعبة (Steam)
          {metaLoading && (
        ...
        <div className="relative">
          ...
          <div className="absolute left-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
            {metaLoading && <Loader2 className="w-4 h-4 animate-spin text-teal-400" />}
      ...
      <main className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-4">
        {loading ? (
          ...
          <div className={viewMode === 'grid'
            ? 'grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 sm:gap-4 max-w-[1800px]'
            : 'space-y-2'
          }>

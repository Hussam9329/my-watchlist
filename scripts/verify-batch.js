/**
 * Verify all sent movies exist in the database
 * Compares the user's list against what's actually in the DB
 */
...
async function main() {
  ...
  const sentMovies = new Map(); // key: title|year, value: {title, year, genre, rating}
  ...
  for (const line of lines) {
    ...
    sentMovies.set(key, { title, year, genre, rating });
  }

  console.log(`Movies sent by user: ${sentMovies.size}`);

  // Fetch ALL rated movies from DB
  console.log('Fetching all movies from DB...');
  const res = await fetch(`${API_BASE}?hasRating=true&type=movie`);
  const data = await res.json();
  const dbMovies = data.items || [];
  console.log(`Movies in DB: ${dbMovies.length}`);
  ...
  // Find movies sent but NOT in DB
  const missing = [];
  for (const [key, movie] of sentMovies) {
    if (!dbMap.has(key)) {
      missing.push(movie);
    }
  }

  // Find movies in DB but NOT in sent list
  const extra = [];
  for (const [key, movie] of dbMap) {
    if (!sentMovies.has(key)) {
      extra.push(movie);
    }
  }

  console.log(`\n=== COMPARISON RESULTS ===`);
  console.log(`Movies you sent: ${sentMovies.size}`);
  console.log(`Movies in DB: ${dbMovies.length}`);
  console.log(`Missing from DB (you sent but not found): ${missing.length}`);
  console.log(`Extra in DB (in DB but not in your list): ${extra.length}`);

  if (missing.length > 0) {
    console.log('\n--- MISSING MOVIES ---');
    missing.forEach(m => console.log(`  ❌ ${m.title} (${m.year}) | ${m.genre} | ${m.rating}`));
  }

  if (extra.length > 0 && extra.length <= 50) {
    console.log('\n--- EXTRA MOVIES IN DB (not in your sent list) ---');
    extra.forEach(m => console.log(`  ➕ ${m.title} (${m.year}) | rating: ${m.userRating}`));
  } else if (extra.length > 50) {
    console.log(`\n--- EXTRA MOVIES IN DB: ${extra.length} (showing first 20) ---`);
    extra.slice(0, 20).forEach(m => console.log(`  ➕ ${m.title} (${m.year}) | rating: ${m.userRating}`));
  }
}
